...
import { BookOpen, LayoutDashboard, ListVideo, Plus, Shield, User, Users } from "lucide-react";
...
export default function Sidebar({ className }: { className?: string }) {
  ...
  const renderGroup = (label: string, items: NavItem[]) => {
    ...
    return (
      <div className="space-y-2">
        <h4 className="px-2 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-600">{label}</h4>
              ...
              >
                <span
                  className={cn(
                    ...
                    active ? "bg-indigo-500/18 text-indigo-200" : "bg-slate-950/45 text-slate-500 group-hover:text-slate-300"
                  )}
                >
                  <item.icon size={16} />
                </span>
  ...
  return (
    <aside className={cn("surface-card flex w-64 shrink-0 flex-col rounded-2xl p-3", className)}>
      <div className="mb-4 rounded-xl border border-slate-800 bg-slate-950/45 p-4">
        ...
        <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.18em] text-indigo-300">{user.role}</p>
      </div>
      <div className="space-y-6 overflow-y-auto pr-1">
        {renderGroup("Main", visible.filter((item) => ["/dashboard", "/profile", "/courses"].includes(item.href)))}
        {renderGroup("Instructor", visible.filter((item) => item.href.startsWith("/teacher")))}
        {renderGroup("System", visible.filter((item) => item.href.startsWith("/admin")))}
      </div>
    </aside>
  );
}
