...
import { BookOpen, GraduationCap, LayoutDashboard, LogOut, Menu, Shield, X } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-slate-800/80 bg-[#07080d]/82 backdrop-blur-2xl">
        ...
        <div className="flex h-16 items-center justify-between gap-4">
          <Link href="/" className="group flex shrink-0 items-center gap-3">
            ...
            <div className="leading-none">
              <span className="block text-lg font-bold tracking-tight text-white">LearnHub</span>
              <span className="hidden text-[10px] font-bold uppercase tracking-[0.22em] text-slate-500 sm:block">Modern LMS</span>
            </div>
          </Link>
          ...
          <div className="hidden items-center gap-3 md:flex">
            {isAuthenticated && user ? (
              <div className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-950/45 py-1.5 pl-2 pr-2">
                <Link href="/profile" className="group flex items-center gap-3">
                  ...
                  <div className="hidden min-w-0 text-left xl:block">
                    <p className="max-w-36 truncate text-xs font-semibold leading-none text-white">{user.name}</p>
                    <Badge label={user.role} variant="role" className="mt-1 py-0.5 text-[9px]" />
                  </div>
                </Link>
                <button
              ...
              <div className="flex items-center gap-2">
                <Link href="/login" className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-300 transition-colors hover:bg-slate-900 hover:text-white">
                  Login
                </Link>
                <Link href="/register" className="rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-950 transition-colors hover:bg-slate-200">
                  Register
                </Link>
              </div>
            )}
        ...
        <div className="absolute left-0 top-16 w-full border-b border-slate-800 bg-[#080910]/96 px-4 py-4 shadow-2xl backdrop-blur-2xl md:hidden">
          ...
          {isAuthenticated && user ? (
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-800 pt-4">
              <Link href="/login" onClick={() => setMobileOpen(false)} className="rounded-xl border border-slate-700 py-3 text-center font-semibold text-white">
                Login
              </Link>
              <Link href="/register" onClick={() => setMobileOpen(false)} className="rounded-xl bg-white py-3 text-center font-bold text-slate-950">
                Register
              </Link>
            </div>
          )}
